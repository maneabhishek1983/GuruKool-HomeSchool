-- Timesheet Tables for QR Check-In/Out System (SAFE VERSION)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'unique_active_parent_student'
  ) THEN
    ALTER TABLE parent_qr_codes
    ADD CONSTRAINT unique_active_parent_student UNIQUE (parent_id, student_id, is_active);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'check_out_after_check_in'
  ) THEN
    ALTER TABLE timesheet_entries
    ADD CONSTRAINT check_out_after_check_in CHECK (
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE timesheet_entries
    ADD CONSTRAINT duration_positive CHECK (
      duration_minutes IS NULL OR duration_minutes >= 0
    );
  END IF;
END $$;

-- Indexes for performance (drop and recreate to ensure they're correct)
DROP INDEX IF EXISTS idx_parent_qr_codes_parent_id;
DROP INDEX IF EXISTS idx_parent_qr_codes_student_id;
DROP INDEX IF EXISTS idx_parent_qr_codes_active;
DROP INDEX IF EXISTS idx_timesheet_entries_teacher_id;
DROP INDEX IF EXISTS idx_timesheet_entries_student_id;
DROP INDEX IF EXISTS idx_timesheet_entries_parent_id;
DROP INDEX IF EXISTS idx_timesheet_entries_status;
DROP INDEX IF EXISTS idx_timesheet_entries_check_in_time;
DROP INDEX IF EXISTS idx_timesheet_entries_teacher_status;
...
-- Trigger to auto-calculate duration (drop and recreate)
...
DROP POLICY IF EXISTS "Parents can view their own QR codes" ON parent_qr_codes;
DROP POLICY IF EXISTS "Parents can insert their own QR codes" ON parent_qr_codes;
DROP POLICY IF EXISTS "Parents can update their own QR codes" ON parent_qr_codes;
DROP POLICY IF EXISTS "Teachers can view QR codes for their students" ON parent_qr_codes;
DROP POLICY IF EXISTS "Teachers can view their own timesheet entries" ON timesheet_entries;
DROP POLICY IF EXISTS "Teachers can insert their own timesheet entries" ON timesheet_entries;
DROP POLICY IF EXISTS "Teachers can update their own timesheet entries" ON timesheet_entries;
DROP POLICY IF EXISTS "Parents can view timesheet entries for their students" ON timesheet_entries;
DROP POLICY IF EXISTS "Admins can view all timesheet entries" ON timesheet_entries;

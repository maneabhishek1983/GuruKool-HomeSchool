    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="p-4 bg-white border border-gray-200 rounded-lg">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(sessionStatus.status)}`}>
            {getStatusIcon(sessionStatus.status)} {sessionStatus.status.replace('-', ' ')}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <span className="ml-2 font-medium">{session?.subject || 'N/A'}</span>
          </div>
          <div>
            <span className="text-gray-600">Teacher:</span>
            <span className="ml-2 font-medium">{session?.teacherName || 'Teacher'}</span>
          </div>
          <div>
            <span className="text-gray-600">Start Time:</span>
            <span className="ml-2 font-medium">
              {session?.scheduledStart ? new Date(session.scheduledStart).toLocaleTimeString() : 'N/A'}
            </span>
          </div>
          <div>
            <span className="text-gray-600">Duration:</span>
            <span className="ml-2 font-medium">
              {sessionStatus.duration > 0 ? formatDuration(sessionStatus.duration) : 'Not started'}
            </span>
          </div>
        </div>
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4 text-sm mb-3">
            <div>
              ...
              <span className="ml-2 font-mono">{teacherLocation.latitude.toFixed(6)}</span>
            </div>
            <div>
              <span className="text-gray-600">Longitude:</span>
              <span className="ml-2 font-mono">{teacherLocation.longitude.toFixed(6)}</span>
            </div>
            <div>
              <span className="text-gray-600">Accuracy:</span>
              <span className="ml-2">±{Math.round(teacherLocation.accuracy)}m</span>
            </div>
            <div>
              <span className="text-gray-600">Distance to Session:</span>
              <span className={`ml-2 font-medium ${
                teacherLocation.isAtSessionLocation ? 'text-green-600' : 'text-red-600'
              }`}>
          ...
          <div className="space-y-2 text-sm">
            ...
            {session?.location?.coordinates && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="text-gray-600">Latitude:</span>
                  <span className="ml-2 font-mono">{session?.location?.coordinates?.latitude?.toFixed(6)}</span>
                </div>
                <div>
                  <span className="text-gray-600">Longitude:</span>
                  <span className="ml-2 font-mono">{session?.location?.coordinates?.longitude?.toFixed(6)}</span>
                </div>
              </div>
            )}
          </div>
      ...
      {isLoading && (
        ...
        >
          ...
          <span className="text-sm text-blue-700 font-medium">Updating location data...</span>
        </motion.div>
      )}

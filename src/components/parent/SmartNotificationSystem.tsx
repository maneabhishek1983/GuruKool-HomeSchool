...
interface NotificationItem {
  ...
  category: 'academic' | 'behavioral' | 'scheduling' | 'communication' | 'system';
  title: string;
  message: string;
  timestamp: Date;
  read: boolean;
...
export default function SmartNotificationSystem({
  ...
  const generateNotifications = (aiInsights: AIRecommendation[], sessions: SessionRecord[]): NotificationItem[] => {
    const items: NotificationItem[] = [];

    // Convert AI insights to notifications
    aiInsights.forEach((insight, index) => {
      const priority = insight.severity === 'high' ? 3 : insight.severity === 'medium' ? 2 : 1;
      const type = priority === 3 ? 'urgent' : priority === 2 ? 'important' : 'informational';
      ...
      items.push({
        ...
        relatedSessionId: sessions.length > 0 ? sessions[0].id : undefined
      });
    });
    ...
    items.push(
      {
        ...
        message: 'Math session scheduled for tomorrow at 3 PM. Teacher will arrive 10 minutes early.',
      ...
      },
      {
        ...
        message: 'Your child is struggling with fractions. Consider scheduling extra practice sessions.',
        timestamp: new Date(Date.now() - 30 * 60 * 1000), // 30 minutes ago
        read: false,
        priority: 3,
        actions: [
          { label: 'Schedule Extra Session', action: 'schedule_extra', type: 'primary' },
          { label: 'View Progress Report', action: 'view_report', type: 'secondary' }
        ],
        aiInsight: 'AI detected learning pattern indicating need for additional support'
      },
      {
        ...
        message: 'Great improvement in focus during reading sessions. Consider positive reinforcement.',
        timestamp: new Date(Date.now() - 2 * 60 * 60 * 1000), // 2 hours ago
        read: true,
        priority: 1,
        actions: [
          { label: 'Send Encouragement', action: 'send_encouragement', type: 'primary' }
        ]
      },
      {
        id: 'communication-1',
        ...
        message: 'Your child completed today\'s homework assignment successfully. Great work!',
  ...
  const determineCategory = (title: string): 'academic' | 'behavioral' | 'scheduling' | 'communication' | 'system' => {
    const lowerTitle = title.toLowerCase();
    if (lowerTitle.includes('math') || lowerTitle.includes('science') || lowerTitle.includes('progress') || lowerTitle.includes('academic')) {
      return 'academic';
    }
    ...
    if (lowerTitle.includes('message') || lowerTitle.includes('communication')) {
  ...
  const handleNotificationAction = (notification: NotificationItem, action: string) => {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={() => handleNotificationAction({} as NotificationItem, 'mark_all_read')}
      ...
      <div className="grid grid-cols-4 gap-4 pt-4 border-t border-gray-200">
        ...
        <div className="text-center">
          <div className="text-lg font-semibold text-yellow-600">
            {notifications.filter(n => n.type === 'important' && !n.read).length}
          </div>
          <div className="text-xs text-gray-500">Important</div>
        </div>
        <div className="text-center">
          <div className="text-lg font-semibold text-blue-600">
            {notifications.filter(n => n.type === 'informational' && !n.read).length}
          </div>

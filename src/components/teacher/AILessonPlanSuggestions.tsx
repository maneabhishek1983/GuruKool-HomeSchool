...
export default function AILessonPlanSuggestions({
  ...
  const loadAISuggestions = async () => {
    try {
      ...
      const recommendations = await enhancedSessionStore.getSessionRecommendations(
        studentId,
        teacherId
      );
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">No AI suggestions available</h3>
          <p className="mt-1 text-sm text-gray-500">
            AI will generate suggestions based on student progress and learning patterns.
          </p>
        </div>
      </motion.div>
    );
  ...
  return (
    ...
    >
        ...
        <AnimatePresence>
          {suggestions.map((suggestion, index) => (
            ...
            >
              ...
              {suggestion.metadata?.resources && suggestion.metadata.resources.length > 0 && (
                <div className="mt-3 pt-3 border-t border-gray-100">
                  <div className="flex items-center space-x-2">
                    <span className="text-xs font-medium text-gray-500">Resources:</span>
                    <div className="flex flex-wrap gap-1">
                      {suggestion.metadata.resources.map((resource: string, idx: number) => (
                        <span
                          key={idx}
      ...
      {selectedSuggestion && (
        ...
        >
          <h4 className="font-medium text-blue-900 mb-2">Selected Suggestion</h4>
          <p className="text-sm text-blue-800 mb-3">{selectedSuggestion.description}</p>

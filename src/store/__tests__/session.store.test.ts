...
import { SchedulingRequest, SessionRecord, SessionQuery } from '@/types/session.types';
    ...
    execute: jest.fn().mockResolvedValue({
      ...
      data: {
        autoNotes: {
          ...
          keyPoints: ['Covered quadratic equations', 'Student showed good understanding'],
        ...
        recommendations: [
          {
            ...
            description: 'Student shows better retention with 45-minute sessions',
            confidence: 0.85,
            priority: 'high',
            actionable: true,
...
describe('EnhancedSessionStore', () => {
  ...
  describe('createSession', () => {
    it('should create a session successfully when no conflicts exist', async () => {
      const result = await sessionStore.createSession(mockSchedulingRequest, mockUser);
    ...
    it('should detect scheduling conflicts and provide alternatives', async () => {
      ...
      const result = await sessionStore.createSession(conflictingRequest, mockUser);
    ...
    it('should generate AI insights for new sessions', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.scheduledSession?.aiInsights).toHaveLength(1);
      expect(result.scheduledSession?.aiInsights[0].content).toContain('Morning sessions work best');
    });
  });
  ...
  describe('updateSession', () => {
    ...
    it('should update session successfully', async () => {
      ...
      const updatedSession = await sessionStore.updateSession(updateRequest, mockUser);
  ...
  describe('getSessionsWithInsights', () => {
    ...
    it('should sort sessions by date', async () => {
      ...
      expect(sessions).toHaveLength(2);
      expect(sessions[0].scheduledStart.getTime()).toBeLessThan(sessions[1].scheduledStart.getTime());
    });
  });
  ...
  describe('getSessionRecommendations', () => {
    ...
    it('should generate AI-powered session recommendations', async () => {
      const recommendations = await sessionStore.getSessionRecommendations('student-1', 'teacher-1');
  ...
  describe('generateAutoNotes', () => {
    ...
    it('should provide fallback notes when AI fails', async () => {
      ...
      const autoNotes = await failingSessionStore.generateAutoNotes(mockSession);
